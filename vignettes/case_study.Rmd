...
We present the case study on Alcohol Consumption of our paper @guerrier2020general. The data is made available in the package, see `?data(student)`. It was originally studied by @cortez2008using. We suppose that the binary reponse `alc` follow a logistic regression model with a misclassified response assuming 5\% of false negative rate. We warn the reader that the code presented here may take a very long time to run on their personal computer. Indeed, for the paper, the computations were performed at University of Geneva on "Baobab" and "Yggdrasil" HPC clusters. 
...
## Real data analysis
The code for the real data analysis is the following:
...
## Numerical study
For the numerical study, we use `beta_tilde` as the true parameter.
