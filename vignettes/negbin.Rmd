...
We present how to perform the simulation studies in @guerrier2020general for negative binomial regression. We warn the reader that the code presented here may take a very long time to run on their personal computer. Indeed, for the paper, the computations were performed at University of Geneva on "Baobab" and "Yggdrasil" HPC clusters. We present the regular negative binomial regression and the negative binomial regression with interfered responses. 
...
Here is the code for the simulation of negative binomial regression:
...
We need first to specify the "interfered process". For that, we need to specify how to generate interfered responses:
```{r interference}
