...
We present how to perform the simulation studies in @guerrier2020general for logistic regression. We warn the reader that the code presented here may take a very long time to run on their personal computer. Indeed, for the paper, the computations were performed at University of Geneva on "Baobab" and "Yggdrasil" HPC clusters. We present the regular logistic regression and the logistic regression with misclassification. 
...
Here is the code for the simulation of logistic regression:
...
We need first to consider the "misclassification process". For that, we need to specify how to generate misclassified responses:
```{r misclassification}
